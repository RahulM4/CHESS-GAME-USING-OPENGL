...
const std::string MSG_YOUR_TURN = "Play";
...
void Game::start(int *argc, char *argv[]) {
    ...
    glutCreateWindow("Chess in 3D using openGL");
...
// Keyboard callback function to handle rotation of the board
void Game::keyboardInput(unsigned char key, int x, int y) {
    switch (key) {
        case 'r': // Press 'r' to rotate the board by 180 degrees
        case 'R':
            cameraAlpha += 3.14159; // Add pi radians to rotate by 180 degrees
            break;
        default:
            break;
    }
...
void Game::init() {
    ...
    glClearColor(0.33, 0.42, 0.67, 0.0);
...
void Game::display() {
    ...
    glMatrixMode(GL_MODELVIEW);
    glLoadIdentity();
        gluLookAt(cameraRadius * cos(cameraAlpha) * cos(cameraBeta), cameraRadius * sin(cameraBeta), cameraRadius * sin(cameraAlpha) * cos(cameraBeta),
              0, 0, 0,
              0, 1, 0);
    // Apply rotation if the rotation flag is set
    if (rotateBoard) {
        glRotatef(180, 0, 1, 0); // Rotate the board by 180 degrees around the Y-axis
        rotateBoard = false; // Reset the rotation flag
    }  
